...
def compute_projected_average(
    cc1: Optional[float] = None,
    cc2: Optional[float] = None,
    regional: Optional[float] = None,
    national_estimated: Optional[float] = None,
) -> Optional[float]:
    """Compute the projected BAC average using the official Moroccan formula.

    Formula (Sciences Expérimentales / Sciences Mathématiques):
        Moyenne BAC = 0.25 * CC + 0.25 * Régional + 0.50 * National

    Where ``CC`` (contrôle continu) is the average of CC 1ère bac and CC 2ème bac.

    All inputs are on a /20 scale. Returns ``None`` if essential data is missing.
    """
    ...
    moyenne = 0.25 * cc + 0.25 * regional + 0.50 * national_estimated
